...
---
## 개요
- 04 Local Repository - add, commit
- 05 Remote Repository - git push origin master, clone, pull(fetch+merge)
...
- **특징**: 깃허브(GitHub) 같은 대부분의 호스팅 서비스가 기본으로 지원한다. 보안을 위해 암호화된 **HTTPS**를 사용한다.

#### (3) SSH
- **개념**: 아이디/비밀번호 대신, 미리 등록된 인증키를 사용해 접속하는 보안이 강화된 방식
- **특징**: 깃(Git)에서 권장하는 방식으로, 한번 설정해두면 비밀번호 입력 없이 안전하게 통신할 수 있어 선호된다.
...
### 3.2. 원격 저장소 등록
- 연결 방법
	1. 먼저 원격 저장소를 생성하고, 이후 로컬 저장소를 연결하는 방법 → [[05 Server#5. 자동으로 내려받기 (clone & pull)|clone & pull]]
	2. 먼저 로컬 저장소를 생성하고, 이후 원격 저장소를 연결하는 방법 → 6장 브랜치 참조
