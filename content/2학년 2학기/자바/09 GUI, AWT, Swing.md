...
2. **main() 메서드 작성**: 프로그램을 시작하는 `main()` 메서드를 작성하며, 일반적으로 프레임 클래스 내부에 만드는 것이 권장된다.
...
- 스윙 프로그램은 `main()` 메서드가 종료되어도 화면이 사라지지 않는데, 이는 GUI와 이벤트를 처리하는 **이벤트 분배 스레드**가 계속 실행 중이기 때문이다.
...
- **배치관리자**(Layout Manager)
	...
	- 각 컨테이너는 기본 배치관리자를 가지며, `Container.setLayout(LayoutManager lm)` 메서드를 통해 변경할 수 있다.
...
- `add(컴포넌트, 인덱스)` 메서드를 사용하여 각 구역에 컴포넌트를 배치한다.
- 컨테이너의 크기가 변하면 **재배치**된다.
...
- 배치관리자를 사용하지 않을 경우, 다음 메서드들을 이용해 컴포넌트의 좌표와 크기를 직접 설정한다.
