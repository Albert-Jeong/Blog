...
- 그레이스 호퍼 (Grace Hopper)
	- **주요 업적**: 컴파일러(Compiler) 개념 최초 고안, 코볼(COBOL) 언어 개발 기여
	- 미 해군 제독 출신의 컴퓨터 과학자로, 기계어 대신 사람이 이해하기 쉬운 프로그래밍 언어가 필요하다는 점을 강조하며 최초의 컴파일러(A-0 시스템)를 개발했다.
	- 이 개념은 훗날 널리 쓰인 프로그래밍 언어인 '코볼'의 설계적 모태가 되었다.
	- 또한 컴퓨터 릴레이에 낀 나방을 발견하고 컴퓨터 오류를 가리키는 '버그(Bug)'와 '디버깅(Debugging)'이라는 용어를 널리 대중화한 인물로도 잘 알려져 있다.
