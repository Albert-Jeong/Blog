...
- **`paintComponent(Graphics g)` 메서드**
	- 스윙 컴포넌트가 자신의 모양을 그릴 때 호출되는 핵심 메서드이다. (`JComponent` 소속)
	- 컴포넌트의 크기나 위치가 변경되거나, 가려졌던 것이 다시 나타날 때 자동으로 호출된다.
	- 사용자가 원하는 모양을 그리려면 이 메서드를 **오버라이딩**하여 구현해야 한다.
...
## 2. Graphics
- **Graphics 객체**: `java.awt.Graphics` 클래스로, 컴포넌트 그리기에 필요한 도구(색상, 폰트, 그리기 메서드 등)를 제공한다.
...
- **도형 칠하기 (내부 채우기)**
	- 도형 그리기 메서드 명의 `draw`를 `fill`로 변경하여 사용한다. (예: `fillRect`, `fillOval` 등)
	- 도형의 외곽선과 내부를 동시에 처리하는 메서드는 없으므로, 필요 시 외곽선 그리기와 칠하기를 각각 호출해야 한다.
...
## 6. 스윙의 페인팅 메커니즘
- **그리기 순서**: `paint()` 메서드가 호출되면 내부적으로 다음 순서대로 메서드들이 전달(`g` 객체 전달)된다.
...
- **재그리기 요청**
	- **`repaint()`**: 컴포넌트의 모양이나 위치가 변경되었을 때 자바 플랫폼에 다시 그리기를 요청하는 메서드 (비동기적 처리)
