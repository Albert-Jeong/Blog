...
- **이벤트 리스너**(Event Listener)
	...
	- JDK는 이벤트 리스너 작성을 위해 인터페이스를 제공하며, 개발자는 이 인터페이스의 추상 메서드를 구현한다.

- **주요 리스너 인터페이스**
	- `ActionListener`: `actionPerformed()` 메서드를 구현하여 `ActionEvent`를 처리한다.
	- `MouseListener`: `mousePressed()`, `mouseReleased()`, `mouseClicked()` 등 5개의 메서드를 구현하여 마우스 이벤트를 처리한다.
	- `KeyListener`: keyPressed(), keyReleased(), keyTyped() 3개의 메서드를 구현하여 키보드 이벤트를 처리한다.
...
- *예제 10-2: 내부 클래스 방식*
	...
	- 외부 클래스의 멤버 변수나 메서드에 쉽게 접근할 수 있는 장점이 있다. 예제에서는 버튼의 텍스트로 프레임의 제목을 변경하는 코드가 추가되었다.
```java
...
- *예제 10-4: MouseListener를 이용한 문자열 이동*
	...
	- `mousePressed` 메서드에서 마우스가 클릭된 좌표를 얻어와 `JLabel`의 위치를 변경한다.
	- `MouseListener`의 모든 추상 메서드(총 5개)를 반드시 구현해야 하므로, 사용하지 않는 메서드들은 빈 채로 둔다.
```java
...
## 4. Adapter 클래스
- **어댑터(Adapter) 클래스**
	- 리스너 인터페이스의 모든 추상 메서드를 빈 상태로 미리 구현해 놓은 클래스
	- `MouseListener`처럼 여러 메서드를 구현해야 하는 인터페이스의 경우, 어댑터 클래스를 상속받으면 원하는 메서드만 오버라이딩(재정의)하여 사용할 수 있어 코드 작성이 간결해진다.

- **예시**: `MouseAdapter`는 `MouseListener`, `MouseMotionListener`, `MouseWheelListener` 인터페이스를 구현한 클래스이다.
- **주의**: `ActionListener`나 `ItemListener`와 같이 추상 메서드가 하나뿐인 인터페이스는 대응하는 어댑터 클래스가 없다.

### 4.1. 예제
- *예제 10-5: MouseAdapter를 이용한 리팩토링*
	- 예제 10-4를 `MouseAdapter` 클래스를 상속받아 개선한 버전
	- `MouseAdapter`는 `MouseListener`의 메서드들이 미리 구현되어 있으므로, 필요한 `mousePressed` 메서드만 오버라이딩(재정의)하면 되어 코드가 더 간결해진다.
```java
...
- *예제 10-8: 방향키로 문자열 이동*
	...
	- `keyPressed` 메서드 안에서 `switch` 문을 사용해 눌린 키의 코드(`getKeyCode`)를 확인하고 각 방향키에 맞는 위치 이동 로직을 수행한다.
...
- *예제 10-10: 더블클릭으로 배경색 변경*
	...
	- `mouseClicked` 이벤트 내에서 `getClickCount()` 메서드의 반환 값이 2인지 확인하여 더블클릭을 감지한다.