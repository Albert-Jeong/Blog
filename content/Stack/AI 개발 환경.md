---
draft: false
---
- **수정 내역**
	- **최초 작성**: 1학년 겨울학기 AI 포트폴리오 프로젝트
	- **1차 수정**: 2학년 여름학기 PyTorch DL

## 1. 하드웨어 환경
- **클라우드/원격 환경**
	- **Google Colab**: 무료 GPU 지원
	- **Kaggle Notebook**: GPU 사용 가능, 대회 중심
	- **AWS, GCP, Azure**: 상용 서비스, 고사양 GPU 사용 가능
	- **Paperspace, Lambda Labs**: 딥러닝 전문 GPU 클라우드

## 2. 소프트웨어 환경
### 2.1. 가상 환경
- **conda**: 패키지 관리 및 환경 관리를 위한 오픈 소스 툴

- **가상 환경의 중요성**
	- 의존성 충돌 방지
	- 재현 가능성 및 이식성
	- 개발 및 디버깅 편의성
	- 자동화 및 배포 용이성

- **주요 명령어**
```shell
conda create -n pytorch python=3.12
conda activate pytorch

conda deactivate

conda info --envs
conda remove --name pytorch --all
```

### 2.2. 개발 언어 및 라이브러리
- **왜 Python인가?**
	- 처리 속도는 느리지만 prototyping은 빠르다.
	- python은 front를 담당하고 C++/CUDA가 핵심 계산인 back을 수행한다.
	- 딥러닝은 행렬 계산이 많기 때문에 numpy 지원이 유리하다.

- **PyTorch**
	- Meta에서 개발한 오픈소스 딥러닝 프레임워크
	- 딥러닝 모델의 개발 및 연구에 사용
	- **GPU 가속 지원**: NVIDIA CUDA 지원을 통해 빠른 연산을 수행
	- **풍부한 생태계**: 다양한 오픈소스 라이브러리(TorchVision, TorchText, TorchAudio)

### 2.3. 호환성
- **250114 AI 포트폴리오 프로젝트 개발 환경 (GTX 1070)**
	- 현재 PyTorch Stable 2.5.1 기준
	- CUDA 11.8, 12.1, 12.4 지원 (12.4 채택)
	- Python 3.9-3.12 지원 (3.12 채택)
```shell
pip3 install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu124

pip3 install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118
```

- **250624 PyTorch DL 개발 환경 (RTX 3060)**
```shell
conda create -n torch-cuda118 python=3.11
conda activate torch-cuda118

pip3 install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118

pip install numpy==1.26 matplotlib==3.8
```

## 3. 개발 도구
- **통합 개발 환경**(IDE)
	- Jupyter Notebook
	- Visual Studio Code

## 4. 버전 관리 및 협업
### 4.1. 버전 관리
- git & github

### 4.2. 협업
- [Notion Workspace](https://sj92.notion.site/summary-and-quiz)

- **노션(Notion)의 주요 장점**
	- 올인원 작업 공간
	- 실시간 협업 가능
	- 유연한 문서 구조
	- 다양한 플랫폼 지원
	- 작업 추적 및 관리 용이